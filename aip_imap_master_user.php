...
<link rel="stylesheet" href="aip_style.css">
<script src="aip_script.js"></script>
</head>
<body>
...
<style>
		...
		#planlotBtnContainerUser { /* user 'Planlot' Btn position */
			position: relative;
			top: -145px; 
			margin-left: 1643px;
		}
		#myInputUser { /* user planlot textbox Input position & size */
			position: relative;
			top: -100.5px; 
			margin-left: 785px;
			width: 850px;
		...
		#AutoJITPivotBtnContainer { /* 'AutoJIT iMap Pivot (AIP)' title position */
			position: relative;
			top: -35px; 
			margin-left: 250px;
		}
    ...
    <form method="post" action="" enctype="multipart/form-data">
        <div>
            <input type="text" id="myInputUser" name="planlotValue" onkeyup="searchPlanlotMasterUser()" placeholder="Filter Planlot | Eg:23311047 | press slash / key to redirect here" style="font-weight: bold; font-size: 1.2em;"/>
            <div id="planlotBtnContainerUser">
                <button type="submit" id="planlotBtn" name="submitplanlot" onclick="checkPlanlotUser()">Planlot</button>
            </div>    
        </div>
    </form>
...
<?php 
    if (isset($_POST['submitplanlot'])) {
        ...
        // $query = "SELECT COUNT(*) AS count FROM autojit_master WHERE joc_no = '$filterValue'";
