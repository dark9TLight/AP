...
function validateForm() {
        ...
        var fileInputab = document.getElementById('upstockab');
        var file = fileInputab.files[0];
...
function validateFormds() { 
    // Get the selected file
    var fileInputdisb = document.getElementById('upstockdisb');
    var file2 = fileInputdisb.files[0];
    ...
    return true; // Allow form submission
        ...
        var dragDropAreab = document.getElementById("dragDropAreab");
        var fileInputab = document.getElementById("upstockab");

        dragDropAreab.addEventListener("dragover", function(e) {
            e.preventDefault();
            dragDropAreab.style.backgroundColor = "#f2f2f2";
        });

        dragDropAreab.addEventListener("dragleave", function(e) {
            e.preventDefault();
            dragDropAreab.style.backgroundColor = "transparent";
        });

        dragDropAreab.addEventListener("drop", function(e) {
            e.preventDefault();
            dragDropAreab.style.backgroundColor = "#008000";
            fileInputab.files = e.dataTransfer.files;
        });

        dragDropAreab.addEventListener("click", function() {
            fileInputab.click();
        });
    ...
    document.addEventListener("DOMContentLoaded", function() {
        var dragDropAreadisb = document.getElementById("dragDropAreadisb");
        var fileInputab = document.getElementById("upstockdisb");

        dragDropAreadisb.addEventListener("dragover", function(e) {
            e.preventDefault();
            dragDropAreadisb.style.backgroundColor = "#f2f2f2";
        });

        dragDropAreadisb.addEventListener("dragleave", function(e) {
            e.preventDefault();
            dragDropAreadisb.style.backgroundColor = "transparent";
        });

        dragDropAreadisb.addEventListener("drop", function(e) {
            e.preventDefault();
            dragDropAreadisb.style.backgroundColor = "#008000";
            fileInputab.files = e.dataTransfer.files;
        });

        dragDropAreadisb.addEventListener("click", function() {
            fileInputab.click();
        });
...
<style>
    ...
    #ImportAutoJITBtndisb { /* Upload AutoJit Btn size */
        width: 137px; 
        height: 50px; 
    }

    #upstockab { /* choose Btn size */
        width: 900px; 
        height: 20px; 
    }
    ...
    #copylinkbtn{ /* 'Copy Link' Btn size  */
        ...
        #chooseFileBtnContainerab { /* drag & drop position */
        position: relative;
        top: -65px;
        /* margin-left: 768px; */
        margin-left: 68px;
        ...
        #chooseFileBtnContainerdisb { /* drag & drop position */
        position: relative;
        top: -678px;
        /* top: -678px; */
        margin-left: 1065px;
        ...
        #ImportAutoJITBtnContainerdisb { /* Upload disb Btn position */
        position: relative;
        top: -672px; 
        margin-left: 1330px;
        }
...
<!-- ----------------------------------------------------------------------------------------------------------------------------------------------- -->
    <form method="post" action="ab_insert_stk.php" enctype="multipart/form-data" onsubmit="return validateForm()"> <!-- ab_algorithm.php -->
        <br>
        <?php 
            $sql = mysqli_query($conn, "SELECT DISTINCT mrp_whs FROM `ab_master`");
            $data = mysqli_fetch_assoc($sql);
        ?>
        <br>
        <div id="chooseFileBtnContainerab">
            <input id="upstockab" type="file" name="stkab" value="Import" style="display: none;">
            <div id="dragDropAreab" style="border: 2px dashed #ccc; padding: 20px; text-align: center; cursor: pointer; width: 900px; height: 480px">
    ...
    <form method="post" action="ab_insert_disb.php" enctype="multipart/form-data" onsubmit="return validateFormds()">
        <br>
        <?php 
            $sql = mysqli_query($conn, "SELECT DISTINCT item_no FROM `ab_disb`");
            $data = mysqli_fetch_assoc($sql);
        ?>
        <br>
        <div id="chooseFileBtnContainerdisb">
            <input id="upstockdisb" type="file" name="stkdisb" value="Import" style="display: none;">
            <div id="dragDropAreadisb" style="border: 2px dashed #ccc; padding: 20px; text-align: center; cursor: pointer; width: 600px; height: 480px">
                <br><br><br><br><br><br><br><br><br><br><p>Drag and drop or click to select a .xls file</p>
            </div>
        </div>
        <div id="ImportAutoJITBtnContainerdisb">
            <button id="ImportAutoJITBtndisb" type="submit" name="import_stkdisb">Upload</button>
        </div>
    </form>
<!-- --------------------------------------------------------------------------------------------------------------------------------------------- -->
    <form method="post" action="ab_master_display.php" enctype="multipart/form-data">
        <div>
...
?>
    <style>
        ...
        #fileNameAutoJIT {
            margin-top: -880px; /* Adjust the top position in pixels */
            margin-left: 88px; /* You can adjust the value as needed */
        }
    </style>
